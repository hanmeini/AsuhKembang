...
import { FaShoppingCart, FaLightbulb, FaStar } from "react-icons/fa";
...
const AffiliateRecommendation = ({ scanResult }) => {
  ...
  const mainIngredient =
    scanResult.aiScanResult.bahan_terdeteksi?.[0]?.nama_bahan;
      ...
      {isHealthy && mainIngredient ? (
        <div className="bg-teal-50 p-4 rounded-lg text-left">
          <h3 className="font-bold text-teal-800">
            Sempurnakan Gizi si Kecil!
          </h3>
          <p className="text-sm text-teal-700 mt-1">
            Bahan utama dalam makanan ini, **{mainIngredient}**, sangat bagus
            untuk pertumbuhan.
          </p>
          <a
            href={searchLink}
        ...
        <div className="bg-yellow-50 p-4 rounded-lg text-center">
          ...
          <p className="text-sm text-yellow-700 mt-1">
            Lihat produk biskuit bayi rendah gula yang direkomendasikan oleh
            ahli gizi kami.
          </p>
          <a
            href={productLink}
    ...
    try {
      ...
      return (
        <div className="space-y-3 mt-2">
          {parsed.intro && (
            <p className="text-xs text-teal-700 font-medium">{parsed.intro}</p>
          )}
          <div className="grid grid-cols-1 gap-2">
            {parsed.segments?.map((seg, i) => (
              <div
                key={i}
                className="bg-white/80 p-2.5 rounded-lg border-l-2 border-teal-400 shadow-sm"
              >
                <h4 className="font-bold text-teal-800 text-[10px] uppercase tracking-wider">
                  {seg.title}
                </h4>
          ...
          {parsed.outro && (
            <p className="text-[10px] text-teal-600 italic">{parsed.outro}</p>
          )}

          {parsed.quotaExceeded && (
            <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
              <p className="text-[10px] text-orange-800 font-bold mb-2 uppercase tracking-tighter flex items-center gap-1">
                <FaStar className="text-orange-500" /> Limit Gratis Tercapai
              </p>
              <Link
                href="/upgrade"
                className="block text-center bg-orange-500 text-white font-bold py-1.5 rounded-md text-[10px] transition-colors shadow-sm hover:bg-orange-600"
              >
                Coba Versi Plus
              </Link>
            </div>
      ...
      // Fallback ke markdown jika parse gagal
    }
  }

  return (
    <div className="text-sm text-teal-600 mt-2 markdown-content">
        ...
        <div>
          ...
          <div className="bg-teal-50 p-4 rounded-lg border border-teal-200">
            <h3 className="font-bold text-teal-700 flex items-center gap-2">
              <span className="text-xl">🥦</span> Rekomendasi Brocco
            </h3>
            <RecommendationBox text={result.recommendation} />
          </div>
          <AffiliateRecommendation scanResult={result} />
        </div>
        <div className="space-y-4">
          <h3 className="text-xl font-bold mb-2">
            {result.aiScanResult?.display_name ?? "-"}
          </h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <NutritionItem
                icon={<FaFire size={20} />}
                label="Kalori"
                value={`${Math.round(nutrition.calories ?? 0)} kkal`}
              />
              <NutritionItem
                icon={<FaDrumstickBite size={20} />}
                label="Protein"
                value={`${Math.round(nutrition.protein ?? 0)} g`}
              />
              <NutritionItem
                icon={<FaCarrot size={20} />}
                label="Karbohidrat"
              ...
              <NutritionItem
                icon={<FaTint size={20} />}
                label="Lemak"
                value={`${Math.round(nutrition.fat ?? 0)} g`}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <NutritionItem
                icon={<AiFillGolden size={20} />}
                label="Zat Besi"
                value={`${Math.round(nutrition.iron ?? 0)} mg`}
              />
              <NutritionItem
                icon={<FaLeaf size={20} />}
                label="Asam Folat"
                value={`${Math.round(nutrition.folic_acid ?? 0)} mcg`}
              />
              <NutritionItem
                icon={<GiSugarCane size={20} />}
                label="Gula"
                value={`${Math.round(nutrition.sugar ?? 0)} g`}
              />
              <NutritionItem
                icon={<GiSaltShaker size={20} />}
                label="Natrium"
                value={`${Math.round(nutrition.sodium ?? 0)} mg`}
              />
            </div>
            ...
            <div className="flex flex-row items-center gap-4 md:gap-8 justify-center">
              ...
              <button
                ...
                onClick={() =>
                  openChatWithContext({ type: "scan_result", data: result })
                }
              >
                Tanya Brocco
              </button>
