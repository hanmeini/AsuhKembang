...
import { FaRegBell, FaHeartbeat, FaUsers, FaUserCircle } from "react-icons/fa";
    ...
    if (userProfile?.profiles?.length > 0) {
      const currentActiveProfile =
        userProfile.profiles.find(
          (p) => p.profileId === userProfile.activeProfileId,
        ) || userProfile.profiles[0];
      setActiveProfile(currentActiveProfile);
    } else if (userProfile) {
      setActiveProfile({
        type: "general",
        name: userProfile.displayName || "Utama",
  ...
  useEffect(() => {
    if (userProfile?.uid && activeProfile) {
      ...
      if (activeProfile?.type === "pregnant" && calculatedWeek) {
        console.log(
          ">>> Memanggil API pregnancy-tip dengan week:",
          calculatedWeek,
        );
        ...
        console.log(
          ">>> Tidak fetch pregnancy-tip karena bukan profil hamil atau minggu null",
        );
      ...
      const unsubscribeWater = subscribeToTodaysWater(
        userProfile.uid,
        (water) => {
  ...
  return (
    <AuthGuard>
      ...
      <div className="">
        ...
        />
        <main
          className={`pb-24 md:pb-0 transition-all duration-300 ease-in-out ${isSidebarExpanded ? "md:pl-[17.5rem]" : "md:pl-[6.5rem]"}`}
        >
          <header className="flex items-start flex-col w-full p-6 md:p-10">
            <div className="flex flex-row justify-between items-center w-full">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800">
                Selamat Pagi, {userProfile?.name || "Pengguna"}! 👋
              </h2>
              <div className="flex items-center gap-4">
                {!userProfile?.isPremium && (
                  <Link
                    href="/upgrade"
                    className="hidden md:flex items-center gap-2 px-4 py-2 rounded-md text-sm text-gray-600 font-semibold bg-white shadow-md hover:shadow-lg transition"
                  >
                    <span>Upgrade to</span>
                    <span className="bg-teal-500 rounded-md px-3 py-1 text-white">
                      Plus
                    </span>
          ...
          <div className="hidden md:grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8 p-2">
              ...
              <div className="bg-teal-500 text-white p-4 md:p-8 md:rounded-2xl shadow-lg flex flex-col md:flex-row items-start gap-4 md:items-center justify-between">
                <div>
                  ...
                  <p className="text-sm md:text-base mt-2 opacity-90">
                    Ayo cari tahu kandungan gizinya sekarang juga.
                  </p>
                </div>
                <Link
                  href="/scan"
                  className="bg-white text-teal-600 text-sm md:text-base font-bold px-6 py-3 rounded-full hover:bg-gray-100 transition-transform hover:scale-105"
                >
                  Mulai Pindai
                </Link>
              </div>
                ...
                <div className="relative bg-white p-8 rounded-2xl shadow-lg flex items-center gap-6 overflow-hidden transition-transform duration-300 hover:scale-[1.02]">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-gray-600">
                      Temukan ribuan resep MPASI dan makanan sehat yang
                      dibagikan oleh para bunda dan divalidasi oleh ahli gizi.
                    </p>
                  </div>
                  <span className="absolute top-4 right-4 text-xs font-semibold bg-pink-100 text-pink-700 px-3 py-1 rounded-full">
                    BARU!
                  </span>
                </div>
              </Link>
              ...
              <div className="p-2 md:p-0">
                <h3 className="text-md md:text-xl font-bold text-gray-700 mb-4">
                  Aktivitas Terakhir
                </h3>
                {todaysScans.length > 0 ? (
                  ...
                  <div className="flex flex-row items-center py-6 px-6 shadow-sm rounded-2xl">
                    <p className="text-gray-500 text-sm">
                      Belum ada makanan yang dipindai hari ini.{" "}
              ...
              <div className="bg-white rounded-2xl shadow-sm flex flex-col md:flex-row">
                <div className="relative w-full h-64 flex-col flex md:mt-0 items-center gap-5 md:gap-14 justify-center group">
                  ...
                  {/* Bubble Chat "Hi" */}
                  <div className="absolute top-1/4 left-0 bg-white px-6 py-3 rounded-full shadow-md opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 ease-in-out transform translate-x-1/4 group-hover:-rotate-3 -translate-y-1/2">
                ...
                <div className="flex justify-center items-start flex-col p-2">
                  <div className="rounded-lg h-auto bg-teal-50 border-l-4 border-teal-500 p-3">
                    <h3 className="font-bold text-teal-800 mb-2">
                      💡 Tip Sehat Hari Ini dari{" "}
                      <span className="text-orange-500">Brocco</span>
                    </h3>
                ...
                {userProfile?.healthProfile ? (
                  ...
                  <div>
                    <p className="text-sm text-gray-500 mt-2">
                      Lengkapi data Anda untuk mendapatkan analisis yang lebih
                      akurat.
                    </p>
                    <Link
                      href="/profile"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      aria-label="minum"
                      onClick={() => handleWaterChange(1)}
                      className="w-7 h-7 bg-blue-500 text-white rounded-full font-bold text-lg flex items-center justify-center hover:bg-blue-600"
                    >
                      +
                    </button>
                  </div>
                </div>
                <p className="text-right text-3xl font-bold text-blue-600 mt-4">
                  {waterIntake}{" "}
          ...
          <div className="block md:hidden space-y-6 px-6 pb-10">
            ...
            <div>
              ...
              <div className="flex overflow-x-auto gap-4 pb-4 px-2 snap-x">
                ...
                >
                  <IoScan size={24} />
                  <span className="text-xs mt-2 leading-tight">
                    Pindai Makanan
                  </span>
                </MotionLink>
                <MotionLink
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Jelajahi resep MPASI & makanan sehat dari para bunda
                  </p>
                </div>
              </div>
            </Link>
